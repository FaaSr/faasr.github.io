...
* A timestamp string is generated in the _entry action of your workflow_ based on the UTC time of the invocation of the action
...
Finally, in the user-provided ID, the invocation ID is provided by the user via the `InvocationID` field in the workflow JSON configuration (set under _Workflow Settings_ in the WebUI). This exact ID is then used by all actions in the workflow.
