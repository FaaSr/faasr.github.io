...
- Using the [FaaSr Workflow Builder Web UI] you can also provide a name for the top-level log folder; if you do not provide one, by default the name is set to `FaaSrLog`
...
Where:
- `ActionName` is the name of the action (i.e. the name of a node in your DAG). For actions invoked concurrently with rank, the log file name also includes the rank, e.g. `ActionName(Rank).txt`
...
- In this workflow configuration, the log folder is the default `FaaSrLog` and the workflow name is `tutorial`
...
You will then be able to retrieve logs for the *sum* action from the (unique) path: 

`FaaSrLog/tutorial/2025-08-29T23-17-01/f81d4fae-7dec-11d0-a765-00a0c91e6bf6/sum.txt`
