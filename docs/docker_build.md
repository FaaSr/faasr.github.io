...
If you need to go beyond adding dependences - for example, compiling a binary from source, building from a non-FaaSr base image, or adding extra build stages - you can write a dedicated Dockerfile and build workflow instead. See [Building a custom image (FLARE example)](custom_image.md) for a complete worked example.
